...
import { getProduct, ProductDelete, UpdateProductsStatus } from "@/services/apiClient";
...
const ProductTaible = () => {
  ...
  useEffect(() => {
    function handleClickOutside(e) {
      if (
        ...
        if (!isMoreButton) setOpenMenuId(null);
      }
    }
  ...
  const handleClick = (product) => {
    if (product.string_status !== "Active") {
      Swal.fire({
        icon: "warning",
  ...
  const handleStatusChange = async (id) => {
    try {
      ...
      const res = await UpdateProductsStatus(id);
      let updatedStatus = res.data.message.success?.[0];

      setProducts((prev) =>
        prev.map((product) => {
          if (product.id === id) {
            const newStatus =
              product.string_status === "Active" ? "Inactive" : "Active";
            return { ...product, string_status: newStatus };
          }
          return product;
        })
      );
      ...
      if (res.data) {
        toast.success(`Product status changed to ${updatedStatus}`);
      }
    } catch (error) {
      console.error(error.message);
              ...
              >
                <td className="py-3 px-6">
                  <div className="w-12 h-12 bg-slate-100 rounded-lg overflow-hidden flex items-center justify-center border border-slate-200">
                    {product.image ? (
                      <Image
                        src={`${baseUrl}/${imagePath}/${product.image}`}
                ...
                <td className="py-4 px-6 text-center">
                  <div className="flex items-center justify-center gap-4">
                    <button
                      ...
                      aria-pressed={product.string_status === "Active"}
                      disabled={loadingStatusId === product.id} // disable during API call
                      className={`relative inline-flex cursor-pointer h-6 w-14 items-center rounded-full transition-all duration-300 ease-in-out ${
                        product.string_status === "Active"
                    ...
                    >
                      {loadingStatusId === product.id ? (
                        ...
                        <span
                          className={`absolute h-4 w-4 rounded-full bg-white transition-all duration-300 ease-in-out shadow-[0_6px_12px_rgba(0,0,0,0.25),inset_0_-2px_4px_rgba(0,0,0,0.15)] ${
                            product.string_status === "Active"
                              ? "translate-x-9"
                              : "translate-x-1"
                          }`}
          ...
          <div className="bg-white rounded-xl w-full max-w-sm p-6">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                className="px-4 py-2 bg-red-600 cursor-pointer text-white rounded-md flex items-center gap-2"
              >
                {deleteLoading && (
